...
static unsigned char * g_output_path="/home/bqu/workspace/DL/yolov3-output/";
...
{ 
    ...
    char *q = strrchr(p,'/') + 1;
    strncpy(name,q,256);//注意后面的6，如果你的测试集的图片的名字字符（不包括后缀）是其他长度，请改为你需要的长度（官方的默认的长度是6）
    return name;
}
...
void test_detector_ori(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    char **names = get_labels(name_list);
...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    ...
    while(1){
        ...
        //resize_network(net, sized.w, sized.h);
            ...
            int m = plist->size;
            if(access(g_output_path,0)==-1)
            {
              if (mkdir(g_output_path,0777))
               {
        ...
        if(outfile){
            ...
            char b[2048];
            sprintf(b,"%s/%s",g_output_path,GetFilename(path));
